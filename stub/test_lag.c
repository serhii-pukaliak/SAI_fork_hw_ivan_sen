...
{
    if (!lst || lst->count != 2 || !lst->list) return 0;
                                  ...
                                  sai_object_list_t *out_list)
{
    ...
    if (!lag_api || !buf || !out_list || buf_cap == 0) return 1;
    ...
    if (status == SAI_STATUS_BUFFER_OVERFLOW) {
        printf("FAIL: LAG PORT_LIST buffer overflow: required=%u cap=%u\n",
               attr[0].value.objlist.count, buf_cap);
        return 1;
    }
    if (status != SAI_STATUS_SUCCESS) {
...
int main(void)
{
    ...
    sai_object_id_t m1 = SAI_NULL_OBJECT_ID, m2 = SAI_NULL_OBJECT_ID, m3 = SAI_NULL_OBJECT_ID, m4 = SAI_NULL_OBJECT_ID;
    ...
    memset(&port_list_view, 0, sizeof(port_list_view));
    ...
    if (status == SAI_STATUS_BUFFER_OVERFLOW) {
        printf("FAIL: SWITCH PORT_LIST buffer overflow: required=%u cap=%u\n",
               sw_attrs[0].value.objlist.count, TEST_MAX_PORTS);
        return 1;
    }
...

    {
        ...
        printf("LAG_MEMBER#3 PORT_ID: " OID_FMT " (expected " OID_FMT ")\n",
               OID_ARG(lm_get[0].value.oid), OID_ARG(port_list[2]));
